...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        echo "<script>alert('Updates applied successfully.');
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['studentId'])) {
    ...
    if ($stmt->affected_rows < 1) {
        echo "Failed to delete student with ID: $studentId";
    } else {
...
// Retrieve student details from the database with search filters
$query = "SELECT students.*, guardians.*
          FROM students
          JOIN guardians ON students.student_id = guardians.student_id";
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['search'])) {
    $searchStudentNo = $_GET['searchStudentNo'];
    $searchGender = $_GET['searchGender'];
    $searchRegisteredDateFrom = $_GET['searchRegisteredDateFrom'];
    $searchRegisteredDateTo = $_GET['searchRegisteredDateTo'];

    if (!empty($searchStudentNo)) {
        $query .= " WHERE students.student_id = $searchStudentNo";
    }

    if (!empty($searchGender)) {
        if (!empty($searchStudentNo)) {
            $query .= " AND students.gender = '$searchGender'";
        } else {
            $query .= " WHERE students.gender = '$searchGender'";
        }
    }

    if (!empty($searchRegisteredDateFrom) && !empty($searchRegisteredDateTo)) {
        $query .= " AND students.registered_date BETWEEN '$searchRegisteredDateFrom' AND '$searchRegisteredDateTo'";
    }
}
        ...
        <div class="row">
            <div class="col-md-4">
                <div class="form-group">
                    <label for="searchStudentNo">Search by Student No:</label>
                    <input type="text" class="form-control" id="searchStudentNo" name="searchStudentNo" placeholder="Enter student number">
                </div>
            </div>
            <div class="col-md-4">
                <div class="form-group">
                    <label for="searchGender">Search by Gender:</label>
            ...
            <div class="col-md-4">
                <div class="form-group">
                    <label for="searchRegisteredDateFrom">Search by Registered Date Range:</label>
                    <div class="input-group">
                        <input type="date" class="form-control" id="searchRegisteredDateFrom" name="searchRegisteredDateFrom">
                        <div class="input-group-prepend input-group-append">
                            <span class="input-group-text">to</span>
                        </div>
                        <input type="date" class="form-control" id="searchRegisteredDateTo" name="searchRegisteredDateTo">
                    </div>
                </div>
            </div>
        </div>
